...
Solves the QHO ground state for a range of grid sizes, then propagates it in
time under a suddenly applied electric field, recording wall-clock time and
...
def make_perturbation(s: iDEA.system.System, t: np.ndarray) -> np.ndarray:
    v_ptrb = np.zeros((t.shape[0], s.x.shape[0]))
...
for N in GRID_SIZES:
    print(f"  N={N}  (Hamiltonian size {N**2}×{N**2}, {TIME_STEPS - 1} timesteps)")
    s = make_qho(N)
    v_ptrb = make_perturbation(s, t)
    gc.collect()
    ...
    try:
        ...
        evolution, peak_gb = _peak_rss_gb(
            lambda: iDEA.methods.interacting.propagate(s, state, v_ptrb, t, GPU=False)  # noqa: B023
        )
...
ax_time.set_title("Propagation time")
...
fig.suptitle(
    f"iDEA interacting time-dependent solver – scaling with grid size (QHO, 2 electrons, {TIME_STEPS - 1} timesteps)"
)
plt.tight_layout()
