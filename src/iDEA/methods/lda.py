...
class HEG:
    ...
    ex_lda = {} 
    ex_lda['spin_polerised'] = {'a' : -1.1511, 'b' : 3.3440, 'c' : -9.7079, 'd' : 19.088, 'e' : -20.896, 'f' : 9.4861, 'g' : 0.73586}
    ec_lda = {} 
    ec_lda['spin_polerised'] = {'a' :  0.0009415195, 'b' :  0.2601, 'c' :  0.06404, 'd' :  0.000248, 'e' :  0.00000261, 'f' :  1.254, 'g' :  28.8}
    vx_lda = {} 
    eps = ex_lda['spin_polerised']
    a, b, c, d, e, f, g = eps['a'], eps['b'], eps['c'], eps['d'], eps['e'], eps['f'], eps['g']
    vx_lda['spin_polerised'] = {'a' : (g+1)*a,'b' : (g+2)*b,'c' : (g+3)*c,'d' : (g+4)*d,'e' : (g+5)*e,'f' : (g+6)*f,'g' : g}
...
def exchange_correlation_potential(s: iDEA.system.System, n: np.ndarray, separate: bool = False) -> np.ndarray:
    ...
    if len(n.shape) == 1:
        ...
        for j in range(s.x.shape[0]):
            if n[j] != 0.0:
                v_x[j] = (p['a'] + p['b']*n[j] + p['c']*n[j]**2 + p['d']*n[j]**3 + p['e']*n[j]**4 + p['f']*n[j]**5)*n[j]**p['g']
                r_s = 0.5 / n[j]
                energy = -((q['a']*r_s + q['e']*r_s**2)/(1.0 + q['b']*r_s + q['c']*r_s**2 + q['d']*r_s**3))*np.log(1.0 + q['f']*r_s + q['g']*r_s**2)/q['f']
                derivative = ((r_s*(q['a'] + q['e']*r_s)*(q['b'] + r_s*(2.0*q['c'] + 3.0*q['d']*r_s))*np.log(1.0 + q['f']*r_s + q['g']*(r_s**2)) - (r_s*(q['a'] + q['e']*r_s)*(q['f'] + 2.0*q['g']*r_s)*(q['b']*r_s + q['c']*(r_s**2) + q['d']*(r_s**3) + 1.0)/(q['f']*r_s + q['g']*(r_s**2) + 1.0)) - ((q['a'] + 2.0*q['e']*r_s)*(q['b']*r_s + q['c']*(r_s**2) + q['d']*(r_s**3) + 1.0)*np.log(1.0 + q['f']*r_s + q['g']*(r_s**2))))/(q['f']*(q['b']*r_s + q['c']*(r_s**2) + q['d']*(r_s**3) + 1.0)**2))
